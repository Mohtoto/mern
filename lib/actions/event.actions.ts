...
export const deleteEvent = async ({eventId , path} : DeleteEventParams) => {
  try {
    ...
    const Deltedevent = await Event.findByIdAndDelete(eventId)

    if (Deltedevent) revalidatePath(path) //clear the cache and re fetch the events

    return JSON.parse(JSON.stringify(Deltedevent));
  } catch (error) {
    handleError(error);
  }
};
